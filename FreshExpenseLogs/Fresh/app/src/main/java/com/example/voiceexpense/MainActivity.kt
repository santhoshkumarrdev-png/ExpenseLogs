...
class MainActivity : ComponentActivity() {
    ...
    private val requestMicPermission = registerForActivityResult(ActivityResultContracts.RequestPermission()) { granted ->
        if (granted) { pendingWebPermissionRequest?.grant(pendingWebPermissionRequest?.resources); pendingWebPermissionRequest = null; webView.reload() }
        ...
        webView = WebView(this).apply {
            ...
            webViewClient = object : WebViewClient() {
                override fun shouldOverrideUrlLoading(view: WebView, request: WebResourceRequest): Boolean {
                    ...
                    return if (host.contains("space-z.ai")) false
                    else { startActivity(Intent(Intent.ACTION_VIEW, request.url.toString().toUri())); true }
                }
            }
            webChromeClient = object : WebChromeClient() {
                override fun onPermissionRequest(request: PermissionRequest) {
                    ...
                    else { pendingWebPermissionRequest = request; requestMicPermission.launch(Manifest.permission.RECORD_AUDIO) }
                }
                override fun onPermissionRequestCanceled(request: PermissionRequest) = request.deny()
            }
